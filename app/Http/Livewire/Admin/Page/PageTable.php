...
class PageTable extends DataTableComponent
{
    ...
    {
        Page::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No page found');
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Action')
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="page" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
