...
class FacilityTable extends DataTableComponent
{
    ...
    {
        Facility::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No facility found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Facility::find((int) $item['value'])->update(['position' => (int) $item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Action')
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="facility" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
