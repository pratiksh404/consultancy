...
class PartnerTable extends DataTableComponent
{
    ...
    {
        return Partner::query()->orderBy('position'); // Eager load anything; // Select some things
    }
    ...
    {
        Partner::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No Partner found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Partner::find((int) $item['value'])->update(['position' => (int) $item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Action')
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="partner" :show="0" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
