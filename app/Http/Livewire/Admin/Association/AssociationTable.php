...
namespace App\Http\Livewire\Admin\Association;
...
class AssociationTable extends DataTableComponent
{
    public function builder(): Builder
    {
        return Association::query()->orderBy('position'); // Eager load anything; // Select some things
    }
    ...
    {
        Association::whereIn('id', $this->getSelected())->delete();
    }
    ...
    public function configure(): void
    {
        ...
        $this->setEmptyMessage('No Association found');
    ...
    public function reorder($items): void
    {
        foreach ($items as $item) {
            Association::find((int) $item['value'])->update(['position' => (int) $item['order']]);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Action')
                ->label(
                    fn ($row, Column $column) => Blade::render('<x-adminetic-action :model="$model" route="association" :show="0" />', ['model' => $row])
                )
                ->html()
                ->collapseOnTablet(),
        ];
